...
def mask_account_card(account_card: str | int | None) -> str:
    ...
    Приоритет: явный счёт ('Счет ' / 'Счёт ') > карта по бренду/длине.
    Для карты с текстовым префиксом (например, 'Visa Classic ') префикс сохраняется.
    """
    ...
    if account_card_text.startswith(ACCOUNT_PREFIX_1) or account_card_text.startswith(ACCOUNT_PREFIX_2):
    ...
    if is_card:
        ...
        first_digit_idx = next((i for i, symbol in enumerate(account_card_text) if symbol.isdigit()), -1)
